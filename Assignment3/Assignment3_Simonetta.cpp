...
// This function gets the base ticket price based on age
double getBasePrice(int age) 
{
    ...
    } else if (age >= 13 && age <= 59) 
    {
        ...
        return -1; // IAge is invalid
    }
}

// This function gets the seat type price adjustment based on user input
...
int main() 
{
    ...
    // Gets the showtime from the user
    validInput = false;
    while (!validInput) 
    {
    ...
    // Applies the showtime discount if applicable
    if (showtime >= 9 && showtime <= 17) 
    {
        basePrice *= 0.90; // 10% discount
    }

    // Gets the seat type from the user using numeric options
    validInput = false;
    while (!validInput) 
    {
    ...
    // Checks if the user has a loyalty card
    validInput = false;
    while (!validInput) 
    {
        ...
        if (loyaltyCard == '1' || loyaltyCard == '0') 
        {
            ...
            if (loyaltyCard == '1') 
            {
                basePrice *= 0.85; // Applies 15% discount
            }
        } else 
        {
    ...
    // Displays the final ticket price with two decimal precision
